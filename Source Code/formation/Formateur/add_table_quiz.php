...
if (isset($_POST["btn_valider"])) {
 ...
 $nom_quiz=escape_dangerous_data($_POST["nom_quiz"]);
 $type_quiz=escape_dangerous_data($_POST["type"]);
 $id_formateur=$_SESSION['id_user_session'];

$q="insert into quiz value('','$nom_quiz',$id_module,$id_chapitre,$id_formateur,'$type_quiz')";
$res=mysqli_query($link,$q)or die(mysqli_error($link));

$q2="SELECT id_quiz FROM quiz ORDER BY  id_quiz  DESC LIMIT 1";
$res2=mysqli_query($link,$q2);
$last_row=mysqli_fetch_array($res2);
$last=$last_row['id_quiz'];


if($type_quiz=="Vrai ou Faux"){
header("Location:ajouter_qst_true_false.php?id_quiz=$last&id_module=$id_module&id_chapitre=$id_chapitre");
}

if($type_quiz=="Choix multiple"){
header("Location:ajouter_qst_choix_mlt.php?id_quiz=$last&id_module=$id_module&id_chapitre=$id_chapitre");
}
   ...
   <div style="margin-top: 15px;margin-right: 20px;">
            <ul class="breadcrumb">
              ...
              <li class="breadcrumb-item active" style="font-size: 16px; font-weight: bold;font-family:'Playfair Display', serif; ">
                ...
                <div class="col-md-4" style="margin-top: 80px; margin-left: 40px; ">
               ...
               <div class="form-group">
                                    <label for="inputEmail3" class="col-sm-3 control-label" style="font-size: 24px;font-family:'Playfair Display', serif;text-align: left; ">Nom Quiz </label>
                                    <div class="col-sm-9">
                                        <div class="form-control-material">
                                            <input type="text" class="form-control" placeholder="Nom Quiz" style="font-size: 22px;" name="nom_quiz"  id="nom_quiz" >
                                            <span id="message_nom_quiz"></span>
                   ...
                   <div class="form-group">
                                    <label for="inputEmail3" class="col-sm-3 control-label"  style="font-size: 24px;font-family:'Playfair Display', serif;text-align: left; ">Type de quiz</label>
                                    <div class="col-sm-9">
...
$('#nom_quiz').focus(function(){
   $('#message_nom_quiz').hide();
});
...
if($('#nom_quiz').val()==''){
   $('#nom_quiz').css("border-color","#DA1900");
   $('#nom_quiz').css("background-color","#F2DEDE");
   $('#message_nom_quiz').html('<span class="text-danger" style="font-size:16px;">Veuillez remplir ce  champs svp </span><br>');
   $('#message_nom_quiz').show();
  valid=false;
}
