...
$texte_reponse=escape_dangerous_data($_POST["texte_reponse"]);
$correcte=escape_dangerous_data($_POST["correcte"]);

$q_reponse_editer_valider="update reponses set reponse='$texte_reponse',          correct=$correcte where id_question=$id_question and id_reponses=$id_reponse ";

mysqli_query($link,$q_reponse_editer_valider)or die(mysqli_error($link));
...
<?php
                ...
                $q_reponse_editer="select * from reponses where id_reponses=$id_reponse and id_question=$id_question ";
                $res_reponse_editer=mysqli_query($link,$q_reponse_editer) or die(mysqli_error($link));
                $row_reponse_editer=mysqli_fetch_assoc($res_reponse_editer) or die(mysqli_error($link));
...
?> 
                ...
                <div class="col-md-6 col-lg-6 " style="margin-top: 0px;margin-left:80px;">            
                    <h1 class="page-section-heading" style="font-family:'Playfair Display', serif; text-align: center; font-size: 37px; margin-left: 40px;margin-top: 0px;">Modifier Réponse</h1>
               ...
               <div class="form-group">
                                    <label for="inputEmail3" class="col-sm-8 control-label" style="font-size: 24px;font-family:'Playfair Display', serif;text-align: left; ">Texte de la réponse </label>
                                    <div class="col-sm-12">
...
$q_type_quiz="select * from quiz where id_quiz=$id_quiz and id_chapitre=$id_chapitre and id_module=$id_module ";

$res_type_quiz=mysqli_query($link,$q_type_quiz) or die(mysqli_error($link));
$row_type_quiz=mysqli_fetch_assoc($res_type_quiz) or die(mysqli_error($link));

$type_quiz=$row_type_quiz["type_quiz"];
...
<?php if($type_quiz=="Choix multiple"){ ?>


                                            <input type="text" class="form-control"  style="font-size: 22px;" name="texte_reponse"
                                            value="<?php echo $row_reponse_editer["reponse"]; ?>" 
                                            >
<?php } ?>

<?php if($type_quiz=="Vrai ou Faux"){ ?>
                                            <input type="text" class="form-control"  style="font-size: 22px;" name="texte_reponse"
                                            value="<?php echo $row_reponse_editer["reponse"]; ?>" readonly="readonly" 
                                            >
...
<?php } ?>
                ...
                <div class="form-group">
                                    <label for="inputEmail3" class="col-sm-3 control-label"  style="font-size: 24px;font-family:'Playfair Display', serif;text-align: left; ">Correcte</label>
                                    <div class="col-sm-9">
                                        ...
                                        <option value="<?php echo $row_reponse_editer["correct"]; ?>" ><?php

                                         if($row_reponse_editer["correct"]==1){
                                          echo "Oui";
                                         }else{
                                          echo "Non";
                                         } ?></option>   
                                         ...
                                         <?php if($row_reponse_editer["correct"]==1){?>
