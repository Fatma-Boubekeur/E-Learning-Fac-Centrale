...
if (isset($_POST["btn_ajouter"])) {
  $nom_formation=escape_dangerous_data($_POST["nom_formation"]);
  $objet_formation=escape_dangerous_data($_POST["objet_formation"]);
  $duree_annee=escape_dangerous_data($_POST["duree_annee"]);
  $q="insert into formation values('','$nom_formation','$objet_formation')";
    ...
    <div class="signup-form">
        <form action="" method="post" id="formulaire" >
             ...
             <div class="form-group">
                   ...
                   <input type="text" class="form-control" name="nom_formation" id="nom_formation"  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Nom de la formation'" placeholder=" Nom de la formation">
                   <span id="message2"></span>
              </div> 

              <div class="form-group">
                   <label >Objet de la formation </label>
                   <textarea name="objet_formation" id="objet_formation" class="form-control" rows="3" cols="5"></textarea>
...
if($('#nom_formation').val()==''){
  $('#nom_formation').css("border-color","#DA1900");
  $('#nom_formation').css("background-color","#F2DEDE");
  $('#message2').html('<span class="text-danger" style="font-size:16px;">Veuillez remplir ce  champs svp </span><br>');
   $('#message2').show();
  valid=false;
}

if($('#objet_formation').val()==''){
  $('#objet_formation').css("border-color","#DA1900");
  $('#objet_formation').css("background-color","#F2DEDE");
  $('#message1').html('<span class="text-danger" style="font-size:16px;">Veuillez remplir ce  champs svp </span><br>');
   $('#message1').show();
  valid=false;
}


return valid && alert(" Formation ajouter ! ");
